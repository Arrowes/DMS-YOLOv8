...
def run(data,
        ...
        name='pcba2',  # save to project/name
    ...
    if not training:
        ...
        sum=1000//(t[0]+t[1]+t[2])
        LOGGER.info(f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS, {sum} FPS, per image at shape {shape}' % t)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'mydata/4/data.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/yolov8n/weights/best.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--name', default='yolov8n', help='save to project/name') #####################name
